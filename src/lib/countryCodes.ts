...
// ITU-assigned international dialing codes, ISO 3166-1 alpha-2 codes. Malta is the default for this Maltese pharmacy.
...
export const DEFAULT_COUNTRY_ISO2 = 'MT';

// Converts an ISO 3166-1 alpha-2 code into its flag emoji using regional indicator symbols
...
export function findCountryByIso2(iso2: string): CountryDialCode {
    return COUNTRY_DIAL_CODES.find((c) => c.iso2 === iso2) ?? COUNTRY_DIAL_CODES.find((c) => c.iso2 === DEFAULT_COUNTRY_ISO2)!;
}

// Splits a stored phone value ("+356 99998888") back into its country and local number.
// Falls back to Malta for legacy records that were saved without a dial code prefix.
export function splitStoredPhone(storedPhone: string): { iso2: string; localNumber: string } {
    const spaceIndex = storedPhone.indexOf(' ');
    if (storedPhone.startsWith('+') && spaceIndex > 0) {
        const dialCode = storedPhone.slice(0, spaceIndex);
        const localNumber = storedPhone.slice(spaceIndex + 1);
        const match = COUNTRY_DIAL_CODES.find((c) => c.dialCode === dialCode);
        if (match) return { iso2: match.iso2, localNumber };
    }
    return { iso2: DEFAULT_COUNTRY_ISO2, localNumber: storedPhone };
}

// Normalizes any stored phone value for display, prefixing legacy dial-code-less records with Malta's code
export function formatDisplayPhone(storedPhone: string): string {
    if (storedPhone.startsWith('+')) return storedPhone;
    return `${findCountryByIso2(DEFAULT_COUNTRY_ISO2).dialCode} ${storedPhone}`;
}
